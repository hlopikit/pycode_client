...
from reusable import batch_api_call, call_api_method
from reusable.batch_api_call import BatchResultDict

from typing import Optional, Union, Any, Tuple


class CallListMethodError(Exception):
...
def _call_list_method(
    ...
    response = call_api_method(
        method, params=fields and fields.copy(),
    )
    ...
    if next_step and total and next_step < total:
        ...
        batch_res = batch_api_call(
            methods=reqs, timeout=timeout,
...
def call_list_method(method, *args, **kwargs):
    result, error = _call_list_method(method, *args, **kwargs)
    if error:
        raise CallListMethodError(error)
